...
public:
	...
	void split(FracTreeNode* father)
	{
		...
		father->left_position.swap(std::vector<int>(father->all_y_size, 0));
		father->right_position.swap(std::vector<int>(father->all_y_size, 0));
		...
		for (int i = 0; i < father->all_y_size;i++)
		{
			if (input_points[points_y[father->all_y[i]]].x < middle_x)
			{
				...
				father->right_position[i] = right_son->all_y_size;
				left_son->all_y_size++;
			}
			else
			{
		...
		if (left_son->all_y_size>1024)//如果有多余1个的点，则继续分裂
		{
			split(left_son);
		}
		if (right_son->all_y_size>1024)//如果有多余1个的点，则继续分裂
		{
			split(right_son);
		}
	}
